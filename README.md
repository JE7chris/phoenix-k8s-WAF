...
* **高性能解耦：** 使用 **Redis** 作为异步消息队列，将流量分析与业务逻辑解耦。
* **全栈可观测性：**
...
graph TD
    ...
    Flask -- 检查黑名单 --> Redis
📸 系统截图1. 安全态势感知大屏 (Real-time Attack Feed)(在此处上传你的大屏截图)2. 基础设施监控 (Grafana)(在此处上传你的 Grafana 截图)🛠️ 技术栈类别技术用途容器编排Kubernetes (Minikube)服务部署、Service 发现、ConfigMap 配置管理容器化Docker镜像构建、多阶段构建开发语言Python 3.9Flask Web 应用、嗅探与分析脚本网络技术Scapy / TCPDump网络包捕获与协议分析中间件Redis消息队列、黑名单缓存数据库MySQL攻击日志持久化存储监控告警Prometheus业务指标采集 (Exporter)数据可视化Grafana系统健康度仪表盘前端ECharts安全数据可视化🚀 快速开始前置要求Kubernetes 集群 (推荐 Minikube)Docker 环境kubectl 命令行工具安装步骤克隆仓库Bashgit clone git@github.com:JE7chris/phoenix-k8s-WAF.git
cd phoenix-k8s-WAF
部署到 KubernetesBash# 一键部署所有组件 (Redis, MySQL, App, Prometheus, Grafana)
kubectl apply -f k8s/
访问控制台WAF 监控大屏： http://<minikube-ip>:30007/dashboardGrafana 面板： http://<minikube-ip>:30300 (默认账号: admin/admin)Prometheus： http://<minikube-ip>:30090攻击测试运行项目自带的攻击脚本，模拟恶意流量：Bashpython3 vm_attacker.py
📂 项目结构
